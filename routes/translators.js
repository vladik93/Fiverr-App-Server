...
router.get('/', (req, res) => {
    ...
    let query3 = "SELECT tr.id, tr.name, tr.lang_from, langfrom.code AS from_code, tr.lang_to, langto.code AS to_code FROM translators AS tr INNER JOIN languages AS langto ON tr.lang_to = langto.id LEFT JOIN languages AS langfrom ON tr.lang_from = langfrom.id"
